...
my $function_getc;        # getc    to getc    or EUCJP::getc
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'EUCJP::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'EUCJP::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'EUCJP::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'EUCJP::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
=item * Returns Next Character
  ...
  If you import getc "use EUCJP qw(getc);", getc of your script will be rewritten in
  EUCJP::getc. EUCJP::getc is not compatible with getc of JPerl.
