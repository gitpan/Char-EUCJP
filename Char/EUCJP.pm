...
######################################################################
#
# Char::EUCJP - Source code filter to EUC-JP script
#
...
sub Char::EUCJP::escape_script {
    ...
    if (/^ use Char::Eeucjp(?:\s+[0-9\.]*)?\s*; $/oxms) {
        ...
        $e_script .= sprintf("use Char::Eeucjp %s;\n%s", $Char::Eeucjp::VERSION, $use_re_eval); # require run-time routines version
...
Char::EUCJP - Source code filter to escape EUC-JP

=head1 SYNOPSIS

  use Char::EUCJP;
  use Char::EUCJP version;         --- require version
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  m?regexp?               m?ditto@Char::Eeucjp::matched?
  s/regexp/replacement/   ($_ =~ m/ditto@Char::Eeucjp::matched/) ?
                          eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  m?regexp?b              m?regexp@Char::Eeucjp::matched?
  s/regexp/replacement/b  ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)@Char::Eeucjp::matched/) ?
                          eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
