...
# regexp of character
my $qq_char   = qr/\\c[\x40-\x5F]|\\?(?:\x8F[\xA1-\xFE][\xA1-\xFE]|[\x8E\xA1-\xFE][\x00-\xFF]|[\x00-\xFF])/oxms;
my  $q_char   = qr/\x8F[\xA1-\xFE][\xA1-\xFE]|[\x8E\xA1-\xFE][\x00-\xFF]|[\x00-\xFF]/oxms;
...
END
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                for (@list) {
                    $Char::Eeucjp::function_ord     = 'Char::EUCJP::ord'     if /\A ord \z/oxms;
                    $Char::Eeucjp::function_ord_    = 'Char::EUCJP::ord_'    if /\A ord \z/oxms;
                    $Char::Eeucjp::function_reverse = 'Char::EUCJP::reverse' if /\A reverse \z/oxms;
                    $Char::Eeucjp::function_getc    = 'Char::EUCJP::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }

    $e_script .= Char::EUCJP::escape();
...
=head1 Multiple-Octet Anchoring of Regular Expression (Char::EUCJP software provides)

Char::EUCJP software applies multiple-octet anchoring at beginning of regular expression.
...
=head1 Multiple-Octet Character Regular Expression (Char::EUCJP software provides)

Char::EUCJP software clusters multiple-octet character with quantifier, makes cluster from
...
=head1 Calling 'Char::Eeucjp::ignorecase()' (Char::EUCJP software provides)

Char::EUCJP software applies calling 'Char::Eeucjp::ignorecase()' instead of /i modifier.
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  s/regexp/replacement/   CORE::eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Eeucjp::matched/) ?
  s/regexp/replacement/   1 :
                          CORE::eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  s/regexp/replacement/b  CORE::eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
...
=head1 Un-Escaping \ of \N, \p, \P, and \X (Char::EUCJP software provides)

Char::EUCJP software removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char::EUCJP software provide)
...
=item * Eval EUC-JP Script
  ...
  $eval = Char::EUCJP::eval { block };
  $eval = Char::EUCJP::eval $expr;
  $eval = Char::EUCJP::eval;

  The Char::EUCJP::eval keyword serves two distinct but related purposes in JPerl.
  These purposes are represented by two forms of syntax, Char::EUCJP::eval { block }
  and Char::EUCJP::eval $expr. The first form traps runtime exceptions (errors)
  ...
  more general. Whichever form you use, Char::EUCJP::eval is the preferred way to do
  all exception handling in JPerl.

  For either form of Char::EUCJP::eval, the value returned from an Char::EUCJP::eval is
  ...
  Char::EUCJP::eval itself. See wantarray for more on how the evaluation context
  can be determined.

  If there is a trappable error (including any produced by the die operator),
  Char::EUCJP::eval returns undef and puts the error message (or object) in $@. If
  ...
  can test it reliably afterward for errors. A simple Boolean test suffices:
      ...
      Char::EUCJP::eval { ... }; # trap runtime errors
      if ($@) { ... }     # handle error

  (Prior to Perl 5.16, a bug caused undef to be returned in list context for
  syntax errors, but not for runtime errors.)

  The Char::EUCJP::eval { block } form is syntax checked and compiled at compile time,
  so it is just as efficient at runtime as any other block. (People familiar
  with the slow Char::EUCJP::eval $expr form are occasionally confused on this issue.)
  Because the { block } is compiled when the surrounding code is, this form of
  Char::EUCJP::eval cannot trap syntax errors.

  The Char::EUCJP::eval $expr form can trap syntax errors because it parses the code
  ...
  after the Char::EUCJP::eval is complete, as do any subroutine or format definitions.
  The code of the Char::EUCJP::eval is treated as a block, so any locally scoped
  variables declared within the Char::EUCJP::eval last only until the Char::EUCJP::eval is
  done. (See my and local.) As with any code in a block, a final semicolon is
  not required.

  Char::EUCJP::eval will be escaped as follows:

  -------------------------------------------------
  Before                  After
  -------------------------------------------------
  Char::EUCJP::eval { block }    eval { block }
  Char::EUCJP::eval $expr        eval Char::EUCJP::escape $expr
  Char::EUCJP::eval              eval Char::EUCJP::escape
  -------------------------------------------------

  To tell the truth, the subroutine Char::EUCJP::eval does not exist. If it exists,
  you will troubled, when Char::EUCJP::eval has a parameter that is single quoted
  string included my variables. Char::EUCJP::escape is a subroutine that makes Perl
  ...
  Since Char::EUCJP::eval traps errors that would otherwise prove fatal, it is useful
  for determining whether particular features (such as fork or symlink) are
  implemented.

  Because Char::EUCJP::eval { block } is syntax checked at compile time, any syntax
  error is reported earlier. Therefore, if your code is invariant and both
  Char::EUCJP::eval $expr and Char::EUCJP::eval { block } will suit your purposes equally
  well, the { block } form is preferred. For example:

      # make divide-by-zero nonfatal
      Char::EUCJP::eval { $answer = $a / $b; };
      warn $@ if $@;
      ...
      Char::EUCJP::eval { $answer = }; # WRONG

      # a runtime syntax error
      Char::EUCJP::eval '$answer =';   # sets $@
  ...
  Using the Char::EUCJP::eval { block } form as an exception trap in libraries does
  ...
  as this example shows:
      ...
      Char::EUCJP::eval { local $SIG{'__DIE__'}; $answer = $a / $b; };
      warn $@ if $@;
  ...
  which has the effect of changing their error messages:
      ...
      # __DIE__ hooks may modify error messages
      {
          ...
          Char::EUCJP::eval { die "foo lives here" };
  ...
  With an Char::EUCJP::eval, you should be especially careful to remember what's being
  looked at when:

      Char::EUCJP::eval $x;        # CASE 1
      Char::EUCJP::eval "$x";      # CASE 2

      Char::EUCJP::eval '$x';      # CASE 3
      Char::EUCJP::eval { $x };    # CASE 4

      Char::EUCJP::eval "\$$x++";  # CASE 5
      $$x++;                # CASE 6
  ...
  a temporary is required if you want to mask some but not all errors:
      ...
      # alter $@ on nefarious repugnancy only
      {
          my $e;
          {
              ...
              Char::EUCJP::eval { test_repugnancy() };
  ...
  The block of Char::EUCJP::eval { block } does not count as a loop, so the loop
...
=head1 Un-Escaping bytes::* Subroutines (Char::EUCJP software provides)

Char::EUCJP software removes 'bytes::' at head of subroutine name.
...
some bugs. Thus, if you happen to find a bug that's in Char::EUCJP software and
