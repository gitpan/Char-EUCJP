...
######################################################################
#
# Char::EUCJP - Source code filter to escape EUC-JP script
#
# http://search.cpan.org/dist/Char-Char::EUCJP/
#
...
my $qq_substr  = qr{(?: Char::EUCJP::substr | CORE::substr | substr ) \( $qq_paren \)
                 }xms;
...
my $matched     = '';
my $s_matched   = '';
$matched        = q{$Char::Eeucjp::matched};
$s_matched      = q{ Char::Eeucjp::s_matched();};
...
my $function_ord;         # ord()   to ord()   or Char::EUCJP::ord()
my $function_ord_;        # ord     to ord     or Char::EUCJP::ord_
my $function_reverse;     # reverse to reverse or Char::EUCJP::reverse
my $function_getc;        # getc    to getc    or Char::EUCJP::getc
...
END
    ...
    print Char::EUCJP::escape_script($ARGV[0]);
    exit 0;
}
...
sub import {
    ...
    if (not -e("$filename.e")) {
        ...
        if (eval q{ use Fcntl qw(O_WRONLY O_APPEND O_CREAT); 1 } and CORE::sysopen($fh,"$filename.e",&O_WRONLY|&O_APPEND|&O_CREAT)) {
            ...
            Char::Eeucjp::_open_a($fh, "$filename.e") or die __FILE__, ": Can't write open file: $filename.e";
        }
        ...
        my $e_script = Char::EUCJP::escape_script($filename);
        print {$fh} $e_script;
    ...
    my $fh = gensym();
    Char::Eeucjp::_open_r($fh, "$filename.e") or die __FILE__, ": Can't read open file: $filename.e";
...
sub Char::EUCJP::escape_script {
    ...
    # read EUC-JP script
    my $fh = gensym();
    Char::Eeucjp::_open_r($fh, $script) or die __FILE__, ": Can't open file: $script";
    local $/ = undef; # slurp mode
    $_ = <$fh>;
    close($fh) or die __FILE__, ": Can't close file: $script";

    if (/^ use Char::Eeucjp(?:\s+[0-9\.]*)?\s*; $/oxms) {
        return $_;
    }
    else {
        ...
        if (s/^ \s* use \s+ Char::EUCJP \s* ([^\x8E\x8F\xA1-\xFE;]*) ; \s* \n? $//oxms) {
            ...
            if ($list =~ s/\A ([0-9]+\.[0-9]+) \.0 \s* //oxms) {
                my $version = $1;
                if ($version ne $Char::EUCJP::VERSION) {
                    my @file = grep -e, map {qq{$_/Char/EUCJP.pm}} @INC;
...
END
                    ...
                    die "Script $0 expects Char/EUCJP.pm $version, but @{[__FILE__]} is version $Char::EUCJP::VERSION\n";
                }
                $e_script .= qq{die "Script \$0 expects Char/Eeucjp.pm $version, but \\\$Char::Eeucjp::VERSION is \$Char::Eeucjp::VERSION" if \$Char::Eeucjp::VERSION ne '$version';\n};
            }
            elsif ($list =~ s/\A ([0-9]+(?:\.[0-9]*)) \s* //oxms) {
                my $version = $1;
                if ($version > $Char::EUCJP::VERSION) {
                    die "Script $0 required Char/EUCJP.pm $version, but @{[__FILE__]} is only version $Char::EUCJP::VERSION\n";
                }
            }
            ...
            if ($list !~ /\A \s* \z/oxms) {
                ...
                my @list = eval $list;
                for (@list) {
                    $function_ord     = 'Char::EUCJP::ord'     if /\A ord \z/oxms;
                    $function_ord_    = 'Char::EUCJP::ord_'    if /\A ord \z/oxms;
                    $function_reverse = 'Char::EUCJP::reverse' if /\A reverse \z/oxms;
                    $function_getc    = 'Char::EUCJP::getc'    if /\A getc \z/oxms;
                }
            }
        }
    }
...
# $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Eeucjp::PREMATCH()
    elsif (/\G ( \$` | \$\{`\} | \$ \s* PREMATCH  \b | \$ \s* \{ \s* PREMATCH  \s* \} | \$ \s* \{\^PREMATCH\}  ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eeucjp::PREMATCH()};
    }

# $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Eeucjp::MATCH()
    elsif (/\G ( \$& | \$\{&\} | \$ \s* MATCH     \b | \$ \s* \{ \s* MATCH     \s* \} | \$ \s* \{\^MATCH\}     ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eeucjp::MATCH()};
    }

# $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Eeucjp::POSTMATCH()
    elsif (/\G (                 \$ \s* POSTMATCH \b | \$ \s* \{ \s* POSTMATCH \s* \} | \$ \s* \{\^POSTMATCH\} ) /oxmsgc) {
        $slash = 'div';
        return q{Char::Eeucjp::POSTMATCH()};
    }
...
# while (<WILDCARD>) --- glob
    ...
    # avoid "Error: Runtime exception" of perl version 5.005_03
        ...
        return 'while ($_ = Char::Eeucjp::glob("' . $1 . '"))';
    }

# while (glob)
    elsif (/\G \b while \s* \( \s* glob \s* \) /oxgc) {
        return 'while ($_ = Char::Eeucjp::glob_)';
    }

# while (glob(WILDCARD))
    elsif (/\G \b while \s* \( \s* glob \b /oxgc) {
        return 'while ($_ = Char::Eeucjp::glob';
    }
...
# subroutines of package Char::Eeucjp
    elsif (/\G \b (CORE:: | ->[ ]* (?: atan2 | [a-z]{2,})) \b  /oxgc) { $slash = 'm//'; return $1;                    }
    elsif (/\G \b bytes::substr \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'substr';              }
    elsif (/\G \b chop \b          (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::chop';         }
    elsif (/\G \b bytes::index \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'index';               }
    elsif (/\G \b Char::EUCJP::index \b   (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::EUCJP::index';         }
    elsif (/\G \b index \b         (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::index';        }
    elsif (/\G \b bytes::rindex \b (?! \s* => )                /oxgc) { $slash = 'm//'; return 'rindex';              }
    elsif (/\G \b Char::EUCJP::rindex \b  (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::EUCJP::rindex';        }
    elsif (/\G \b rindex \b        (?! \s* => )                /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::rindex';       }
    elsif (/\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::lc';           }
    elsif (/\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::lcfirst';      }
    elsif (/\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::uc';           }
    elsif (/\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::ucfirst';      }
    elsif (/\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::fc';           }
    ...
    elsif (/\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::chr';               }
    elsif (/\G \b bytes::ord    (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'div'; return $function_ord;              }
    elsif (/\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::glob';              }
    elsif (/\G \b lc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::lc_';               }
    elsif (/\G \b lcfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::lcfirst_';          }
    elsif (/\G \b uc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::uc_';               }
    elsif (/\G \b ucfirst \b    (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::ucfirst_';          }
    elsif (/\G \b fc \b         (?! \s* => )                         /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::fc_';               }
    ...
    elsif (/\G \b chr \b           (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::chr_';              }
    elsif (/\G \b bytes::ord \b    (?! \s* => )                      /oxgc) { $slash = 'div'; return 'ord';                      }
    elsif (/\G \b ord \b           (?! \s* => )                      /oxgc) { $slash = 'div'; return $function_ord_;             }
    elsif (/\G \b glob \b          (?! \s* => )                      /oxgc) { $slash = 'm//'; return 'Char::Eeucjp::glob_';             }
...
# end of split
        if    (/\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Eeucjp::split' . $e;                 }

# split scalar value
        elsif (/\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { return 'Char::Eeucjp::split' . $e . e_string($1);  }

# split literal space
        elsif (/\G \b qq       (\#) [ ] (\#) /oxgc) { return 'Char::Eeucjp::split' . $e . qq  {qq$1 $2}; }
        elsif (/\G \b qq (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; }
        elsif (/\G \b q        (\#) [ ] (\#) /oxgc) { return 'Char::Eeucjp::split' . $e . qq   {q$1 $2}; }
        elsif (/\G \b q  (\s*) (\() [ ] (\)) /oxgc) { return 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { return 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { return 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { return 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { return 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; }
        elsif (/\G                ' [ ] '    /oxgc) { return 'Char::Eeucjp::split' . $e . qq     {' '};  }
        elsif (/\G                " [ ] "    /oxgc) { return 'Char::Eeucjp::split' . $e . qq     {" "};  }
...
# subroutines of package Char::Eeucjp
        ...
        elsif ($string =~ /\G \b chop \b                                      /oxgc) { $e_string .= 'Char::Eeucjp::chop';    $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::index \b                              /oxgc) { $e_string .= 'index';          $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::EUCJP::index \b                               /oxgc) { $e_string .= 'Char::EUCJP::index';    $slash = 'm//'; }
        elsif ($string =~ /\G \b index \b                                     /oxgc) { $e_string .= 'Char::Eeucjp::index';   $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::rindex \b                             /oxgc) { $e_string .= 'rindex';         $slash = 'm//'; }
        elsif ($string =~ /\G \b Char::EUCJP::rindex \b                              /oxgc) { $e_string .= 'Char::EUCJP::rindex';   $slash = 'm//'; }
        elsif ($string =~ /\G \b rindex \b                                    /oxgc) { $e_string .= 'Char::Eeucjp::rindex';  $slash = 'm//'; }
        elsif ($string =~ /\G \b lc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeucjp::lc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeucjp::lcfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b uc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeucjp::uc';      $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeucjp::ucfirst'; $slash = 'm//'; }
        elsif ($string =~ /\G \b fc      (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeucjp::fc';      $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr           (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeucjp::chr';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b glob          (?= \s+[A-Za-z_]|\s*['"`\$\@\&\*\(]) /oxgc) { $e_string .= 'Char::Eeucjp::glob';              $slash = 'm//'; }
        elsif ($string =~ /\G \b lc \b                                              /oxgc) { $e_string .= 'Char::Eeucjp::lc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b lcfirst \b                                         /oxgc) { $e_string .= 'Char::Eeucjp::lcfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b uc \b                                              /oxgc) { $e_string .= 'Char::Eeucjp::uc_';               $slash = 'm//'; }
        elsif ($string =~ /\G \b ucfirst \b                                         /oxgc) { $e_string .= 'Char::Eeucjp::ucfirst_';          $slash = 'm//'; }
        elsif ($string =~ /\G \b fc \b                                              /oxgc) { $e_string .= 'Char::Eeucjp::fc_';               $slash = 'm//'; }
        ...
        elsif ($string =~ /\G \b chr \b                                             /oxgc) { $e_string .= 'Char::Eeucjp::chr_';              $slash = 'm//'; }
        elsif ($string =~ /\G \b bytes::ord \b                                      /oxgc) { $e_string .= 'ord';                      $slash = 'div'; }
        elsif ($string =~ /\G \b ord \b                                             /oxgc) { $e_string .= $function_ord_;             $slash = 'div'; }
        elsif ($string =~ /\G \b glob \b                                            /oxgc) { $e_string .= 'Char::Eeucjp::glob_';             $slash = 'm//'; }
...
# end of split
            if    ($string =~ /\G (?= [,;\)\}\]] )          /oxgc) { return 'Char::Eeucjp::split' . $e;                                           }

# split scalar value
            elsif ($string =~ /\G ( [\$\@\&\*] $qq_scalar ) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . e_string($1);  next E_STRING_LOOP; }

# split literal space
            elsif ($string =~ /\G \b qq       (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq  {qq$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b qq (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq{$1qq$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q        (\#) [ ] (\#) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq   {q$1 $2}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\() [ ] (\)) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\{) [ ] (\}) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\[) [ ] (\]) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\<) [ ] (\>) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G \b q  (\s*) (\S) [ ] (\2) /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq {$1q$2 $3}; next E_STRING_LOOP; }
            elsif ($string =~ /\G                ' [ ] '    /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq     {' '};  next E_STRING_LOOP; }
            elsif ($string =~ /\G                " [ ] "    /oxgc) { $e_string .= 'Char::Eeucjp::split' . $e . qq     {" "};  next E_STRING_LOOP; }
...
sub character_class {
    ...
    if ($char eq '.') {
        if ($modifier =~ /s/) {
            return '${Char::Eeucjp::dot_s}';
        }
        else {
            return '${Char::Eeucjp::dot}';
        }
    }
    else {
        return Char::Eeucjp::classic_character_class($char);
    }
}
...
sub e_capture {
    ...
    return join '', '${Char::Eeucjp::capture(', $_[0], ')}';
    return join '', '$',                 $_[0];
}
...
sub e_tr {
    ...
    if ($modifier =~ tr/bB//d) {
        ...
        if ($variable eq '') {
            $e_tr = qq{Char::Eeucjp::tr(\$_,' =~ ',$charclass,$e$charclass2,'$modifier')};
        }
        else {
            $e_tr = qq{Char::Eeucjp::tr($variable,'$bind_operator',$charclass,$e$charclass2,'$modifier')};
        }
    }
...
sub e_qq {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # octal escape sequence
            ...
            $char[$i] = Char::Eeucjp::octchr($1);
        }

        # hexadecimal escape sequence
        elsif ($char[$i] =~ /\A \\x \{ ([0-9A-Fa-f]+) \} \z/oxms) {
            $char[$i] = Char::Eeucjp::hexchr($1);
        }
        ...
        # \u \l \U \L \F \Q \E
            ...
            $char[$i] = '@{[Char::Eeucjp::ucfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\l') {
            $char[$i] = '@{[Char::Eeucjp::lcfirst qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\U') {
            $char[$i] = '@{[Char::Eeucjp::uc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\L') {
            $char[$i] = '@{[Char::Eeucjp::lc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\F') {
            $char[$i] = '@{[Char::Eeucjp::fc qq<';
            $left_e++;
        }
        elsif ($char[$i] eq '\Q') {
...
sub e_qr {
    ...
    for (my $i=0; $i <= $#char; $i++) {
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Eeucjp::charlist_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Eeucjp::charlist_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # open character class [^...]
            ...
            while (1) {
                ...
                if ($char[$i] eq ']') {
                    ...
                    # [^...]
                    if (grep(/\A [\$\@]/oxms,@char[$left+1..$right-1]) >= 1) {
                        splice @char, $left, $right-$left+1, sprintf(q{@{[Char::Eeucjp::charlist_not_qr(%s,'%s')]}}, join(',', map {qq_stuff($delimiter,$end_delimiter,$_)} @char[$left+1..$right-1]), $modifier);
                    }
                    else {
                        splice @char, $left, $right-$left+1, Char::Eeucjp::charlist_not_qr(@char[$left+1..$right-1], $modifier);
                    }
        ...
        # /i modifier
        elsif ($ignorecase and ($char[$i] =~ /\A [\x00-\xFF] \z/oxms) and (Char::Eeucjp::uc($char[$i]) ne Char::Eeucjp::fc($char[$i]))) {
            if (CORE::length(Char::Eeucjp::fc($char[$i])) == 1) {
                $char[$i] = '['   . Char::Eeucjp::uc($char[$i])       . Char::Eeucjp::fc($char[$i]) . ']';
            }
            else {
                $char[$i] = '(?:' . Char::Eeucjp::uc($char[$i]) . '|' . Char::Eeucjp::fc($char[$i]) . ')';
            }
        ...
        # $0 --> $0
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeucjp::ignorecase(' . $char[$i] . ')]}';
            }
        }
        elsif ($char[$i] =~ /\A \$ \{ \s* 0 \s* \} \z/oxms) {
            if ($ignorecase) {
        ...
        # $`, ${`}, $PREMATCH, ${PREMATCH}, ${^PREMATCH} --> Char::Eeucjp::PREMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeucjp::ignorecase(Char::Eeucjp::PREMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eeucjp::PREMATCH()]}';
            }
        ...
        # $&, ${&}, $MATCH, ${MATCH}, ${^MATCH} --> Char::Eeucjp::MATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeucjp::ignorecase(Char::Eeucjp::MATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eeucjp::MATCH()]}';
            }
        ...
        # $POSTMATCH, ${POSTMATCH}, ${^POSTMATCH} --> Char::Eeucjp::POSTMATCH()
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeucjp::ignorecase(Char::Eeucjp::POSTMATCH())]}';
            }
            else {
                $char[$i] = '@{[Char::Eeucjp::POSTMATCH()]}';
            }
...
sub e_sub {
    ...
    if ($r_modifier) {
        ...
        # s///gr with multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                                              2   3                                 4   5
                q<eval{local $Char::EUCJP::re_t=%s; local $Char::EUCJP::re_a=''; while($Char::EUCJP::re_t =~ %s){%s local $^W=0; local $Char::EUCJP::re_r=%s; %s$Char::EUCJP::re_t="$Char::EUCJP::re_a${1}$Char::EUCJP::re_r$'"; pos($Char::EUCJP::re_t)=length "$Char::EUCJP::re_a${1}$Char::EUCJP::re_r"; $Char::EUCJP::re_a=substr($Char::EUCJP::re_t,0,pos($Char::EUCJP::re_t)); } return $Char::EUCJP::re_t}>,
                ...
                $s_matched,                                                      #  3
                $e_replacement,                                                  #  4
                '$Char::EUCJP::re_r=eval $Char::EUCJP::re_r; ' x $e_modifier,                  #  5
            );
        }
        ...
        # s///gr without multibyte anchoring
            ...
            $sub = sprintf(
                #                        1                        2   3                                 4   5
                q<eval{local $Char::EUCJP::re_t=%s; while($Char::EUCJP::re_t =~ %s){%s local $^W=0; local $Char::EUCJP::re_r=%s; %s$Char::EUCJP::re_t="$`$Char::EUCJP::re_r$'"; pos($Char::EUCJP::re_t)=length "$`$Char::EUCJP::re_r"; } return $Char::EUCJP::re_t}>,
        ...
        # s///r
            ...
            $sub = sprintf(
                #  1     2          3                                 4   5  6                    7
                q<(%s =~ %s) ? eval{%s local $^W=0; local $Char::EUCJP::re_r=%s; %s"%s$Char::EUCJP::re_r$'" } : %s>,
    ...
    # without /r
        ...
        # s///g with multibyte anchoring
            ...
            $sub = sprintf(
                ...
                q<eval{local $Char::EUCJP::re_n=0; local $Char::EUCJP::re_a=''; while(%s =~ %s){%s local $^W=0; local $Char::EUCJP::re_r=%s; %s%s="$Char::EUCJP::re_a${1}$Char::EUCJP::re_r$'"; pos(%s)=length "$Char::EUCJP::re_a${1}$Char::EUCJP::re_r"; $Char::EUCJP::re_a=substr(%s,0,pos(%s)); $Char::EUCJP::re_n++} return %s$Char::EUCJP::re_n}>,
...
# (and so on)
        ...
        # s///g without multibyte anchoring
            ...
            $sub = sprintf(
                #                                 1     2   3                                 4   5 6                         7                                                 8
                q<eval{local $Char::EUCJP::re_n=0; while(%s =~ %s){%s local $^W=0; local $Char::EUCJP::re_r=%s; %s%s="$`$Char::EUCJP::re_r$'"; pos(%s)=length "$`$Char::EUCJP::re_r"; $Char::EUCJP::re_n++} return %s$Char::EUCJP::re_n}>,
        ...
        # s///
            ...
            $sub = sprintf(
                ...
                #  1 2 3          4                                 5   6 7   8
                q<(%s%s%s) ? eval{%s local $^W=0; local $Char::EUCJP::re_r=%s; %s%s="%s$Char::EUCJP::re_r$'"; 1 } : undef> :

                #  1 2 3              4                                 5   6 7   8
                q<(%s%s%s) ? 1 : eval{%s local $^W=0; local $Char::EUCJP::re_r=%s; %s%s="%s$Char::EUCJP::re_r$'"; undef }>,
                ...
                $s_matched,                                                      #  4
                $e_replacement,                                                  #  5
                '$Char::EUCJP::re_r=eval $Char::EUCJP::re_r; ' x $e_modifier,                  #  6
                $variable,                                                       #  7
                $prematch,                                                       #  8
            );
        }
...
sub e_split {
    ...
    for (my $i=0; $i <= $#char; $i++) {
        ...
        # ${ foo }
            ...
            if ($ignorecase) {
                $char[$i] = '@{[Char::Eeucjp::ignorecase(' . $1 . ')]}';
            }
        }
    ...
    $modifier =~ tr/i//d;
    if ($left_e > $right_e) {
        return join '', 'Char::Eeucjp::split', $ope, $delimiter, @char, '>]}' x ($left_e - $right_e), $end_delimiter, $modifier;
    }
    return     join '', 'Char::Eeucjp::split', $ope, $delimiter, @char,                               $end_delimiter, $modifier;
}
...
=item * You'll have to download Char/EUCJP.pm and Char/Eeucjp.pm and put it in your perl lib directory.

=item * You'll need to write "use Char::EUCJP;" at head of the script.
...
=head1 SYNOPSIS
  ...
  use Char::EUCJP;
  use Char::EUCJP ver.sion;             --- require minimum version
  use Char::EUCJP ver.sion.0;           --- expects version (match or die)
  use Char::EUCJP qw(ord reverse getc); --- demand enhanced feature of ord, reverse, and getc
  use Char::EUCJP ver.sion qw(ord reverse getc);
  use Char::EUCJP ver.sion.0 qw(ord reverse getc);

  # "no Char::EUCJP;" not supported

  or

  $ perl Char/EUCJP.pm EUC-JP_script.pl > Escaped_script.pl.e
...
Char::EUCJP software is "middleware" between perl interpreter and your Perl script
written in EUC-JP.
...
By "use Char::EUCJP;", it automatically interpret your script as EUC-JP. The various
...
=head1 JRE: JPerl Runtime Environment
  ...
  +---------------------------------------+
  |  Source Code Filter, Runtime Routine  | ex. Char/EUCJP.pm, Char/Eeucjp.pm
  +---------------------------------------+
...
expand wildcard arguments supplied to programs. Instead, import of Char/Eeucjp.pm
works well.
   ...
   sub import {
       ...
       if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
           ...
           for (@ARGV) {
               ...
               # has space
               if (/\A (?:$q_char)*? [ ] /oxms) {
                   if (my @glob = Char::Eeucjp::glob(qq{"$_"})) {
                       push @argv, @glob;
                   }
                   else {
                       push @argv, $_;
...
=head1 Software Composition
   ...
   Char/EUCJP.pm               --- source code filter to escape EUC-JP
   Char/Eeucjp.pm              --- run-time routines for Char/EUCJP.pm
...
You need write 'use Char::EUCJP;' in your script.

  ---------------------------------
  Before      You do
  ---------------------------------
  (nothing)   use Char::EUCJP;
  ---------------------------------

=head1 Multiple-Octet Anchoring of Regular Expression (Char/EUCJP.pm provides)

Char/EUCJP.pm applies multiple-octet anchoring at beginning of regular expression.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/regexp/               m/${Char::Eeucjp::anchor}(?:regexp).../
  --------------------------------------------------------------------------------

=head1 Multiple-Octet Character Regular Expression (Char/EUCJP.pm provides)

Char/EUCJP.pm clusters multiple-octet character with quantifier, makes cluster from
...
from classic Perl character class shortcuts and POSIX-style character classes.
  ...
  m/...\D.../             m/...(?:${Char::Eeucjp::eD}).../
  m/...[[:^digit:]].../   m/...(?:${Char::Eeucjp::not_digit}).../
  --------------------------------------------------------------------------------

=head1 Calling 'Char::Eeucjp::ignorecase()' (Char/EUCJP.pm provides)

Char/EUCJP.pm applies calling 'Char::Eeucjp::ignorecase()' instead of /i modifier.

  --------------------------------------------------------------------------------
  Before                  After
  --------------------------------------------------------------------------------
  m/...$var.../i          m/...@{[Char::Eeucjp::ignorecase($var)]}.../
  --------------------------------------------------------------------------------
...
Regular expression works as character-oriented that has no /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/                /ditto$Char::Eeucjp::matched/
  m/regexp/               m/ditto$Char::Eeucjp::matched/
  ?regexp?                m?ditto$Char::Eeucjp::matched?
  m?regexp?               m?ditto$Char::Eeucjp::matched?
 
  $_ =~                   ($_ =~ m/ditto$Char::Eeucjp::matched/) ?
  s/regexp/replacement/   eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
 
  $_ !~                   ($_ !~ m/ditto$Char::Eeucjp::matched/) ?
  s/regexp/replacement/   1 :
                          eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; undef }
 
  split(/regexp/)         Char::Eeucjp::split(qr/regexp/)
  split(m/regexp/)        Char::Eeucjp::split(qr/regexp/)
  split(qr/regexp/)       Char::Eeucjp::split(qr/regexp/)
  qr/regexp/              qr/ditto$Char::Eeucjp::matched/
  --------------------------------------------------------------------------------
...
Regular expression works as byte-oriented that has /b modifier.
  ...
  Before                  After
  --------------------------------------------------------------------------------
  /regexp/b               /(?:regexp)$Char::Eeucjp::matched/
  m/regexp/b              m/(?:regexp)$Char::Eeucjp::matched/
  ?regexp?b               m?regexp$Char::Eeucjp::matched?
  m?regexp?b              m?regexp$Char::Eeucjp::matched?
 
  $_ =~                   ($_ =~ m/(\G[\x00-\xFF]*?)(?:regexp)$Char::Eeucjp::matched/) ?
  s/regexp/replacement/b  eval{ Char::Eeucjp::s_matched(); local $^W=0; my $__r=qq/replacement/; $_="${1}$__r$'"; 1 } :
                          undef
  ...
  qr/regexp/b             qr/(?:regexp)$Char::Eeucjp::matched/
  --------------------------------------------------------------------------------

=head1 Escaping Character Classes (Char/Eeucjp.pm provides)
...
Definitions in Char/Eeucjp.pm.
  ...
  ${Char::Eeucjp::eS}             qr{(?:\x8F[\xA1-\xFE][\xA1-\xFE]|[\x8E\xA1-\xFE][\x00-\xFF]|[^\x8E\x8F\xA1-\xFE\s])}
  ...
  ${Char::Eeucjp::not_space}      qr{(?:\x8F[\xA1-\xFE][\xA1-\xFE]|[\x8E\xA1-\xFE][\x00-\xFF]|[^\x8E\x8F\xA1-\xFE\s\x0B])}
...
=head1 Un-Escaping \ of \N, \p, \P and \X (Char/EUCJP.pm provides)

Char/EUCJP.pm removes '\' at head of alphanumeric regexp metasymbols \N, \p, \P
...
=head1 Escaping Built-in Functions (Char/EUCJP.pm and Char/Eeucjp.pm provide)

Insert 'Char::Eeucjp::' at head of function name. Char/Eeucjp.pm provides your script Char::Eeucjp::*
subroutines.
  ...
  substr      substr           Byte
  pos         pos              Byte
  split       Char::Eeucjp::split     Character
  tr///       Char::Eeucjp::tr        Character
  tr///b      tr///            Byte
  tr///B      tr///            Byte
  y///        Char::Eeucjp::tr        Character
  y///b       tr///            Byte
  y///B       tr///            Byte
  chop        Char::Eeucjp::chop      Character
  index       Char::Eeucjp::index     Character
  rindex      Char::Eeucjp::rindex    Character
  lc          Char::Eeucjp::lc        Character
  lcfirst     Char::Eeucjp::lcfirst   Character
  uc          Char::Eeucjp::uc        Character
  ucfirst     Char::Eeucjp::ucfirst   Character
  fc          Char::Eeucjp::fc        Character
  chr         Char::Eeucjp::chr       Character
  glob        Char::Eeucjp::glob      Character
  -------------------------------------------
...
You need write 'Char::EUCJP::' at head of function name when you want character-
oriented subroutine. See 'Character-Oriented Subroutines'.
  ...
  --------------------------------------------------------
  ord        Char::EUCJP::ord
  reverse    Char::EUCJP::reverse
  getc       Char::EUCJP::getc
  length     Char::EUCJP::length
  substr     Char::EUCJP::substr
  index      Char::EUCJP::index          See 'About Indexes'
  rindex     Char::EUCJP::rindex         See 'About Rindexes'
  --------------------------------------------------------
  ...
  (same as Char::Eeucjp::index)
  Char::EUCJP::index    Character   Character    Character-oriented semantics
  ...
  (same as Char::Eeucjp::rindex)
  Char::EUCJP::rindex   Character   Character    Character-oriented semantics
...
=item * Ordinal Value of Character
  ...
  If you import ord "use Char::EUCJP qw(ord);", ord of your script will be rewritten in
  Char::EUCJP::ord. Char::EUCJP::ord is not compatible with ord of JPerl.

=item * Reverse List or String

  @reverse = Char::EUCJP::reverse(@list);
  $reverse = Char::EUCJP::reverse(@list);
  ...
  If you import reverse "use Char::EUCJP qw(reverse);", reverse of your script will be
  rewritten in Char::EUCJP::reverse. Char::EUCJP::reverse is not compatible with reverse of
  JPerl.
...
=item * Returns Next Character
  ...
  $getc = Char::EUCJP::getc(FILEHANDLE);
  $getc = Char::EUCJP::getc($filehandle);
  $getc = Char::EUCJP::getc;
  ...
  If you import getc "use Char::EUCJP qw(getc);", getc of your script will be rewritten
  in Char::EUCJP::getc. Char::EUCJP::getc is not compatible with getc of JPerl.

=item * Length by EUC-JP Character

  $length = Char::EUCJP::length($string);
  $length = Char::EUCJP::length();

  This subroutine returns the length in characters (programmer-visible characters)
  of the scalar value $string. If $string is omitted, it returns the Char::EUCJP::length
  of $_.

  Do not try to use Char::EUCJP::length to find the size of an array or hash. Use scalar
...
=item * Substr by EUC-JP Character
  ...
  $substr = Char::EUCJP::substr($string,$offset,$length,$replacement);
  $substr = Char::EUCJP::substr($string,$offset,$length);
  $substr = Char::EUCJP::substr($string,$offset);
  ...
  my $color  = Char::EUCJP::substr $s, 4, 5;      # black
  my $middle = Char::EUCJP::substr $s, 4, -11;    # black cat climbed the
  my $end    = Char::EUCJP::substr $s, 14;        # climbed the green tree
  my $tail   = Char::EUCJP::substr $s, -4;        # tree
  my $z      = Char::EUCJP::substr $s, -4, 2;     # tr

  If Perl version 5.14 or later, you can use the Char::EUCJP::substr() subroutine as an
  ...
  end of the string, Char::EUCJP::substr() returns the undefined value and produces a
  ...
  my $name = 'fred';
  Char::EUCJP::substr($name, 4) = 'dy';         # $name is now 'freddy'
  my $null = Char::EUCJP::substr $name, 6, 2;   # returns "" (no warning)
  my $oops = Char::EUCJP::substr $name, 7;      # returns undef, with warning
  Char::EUCJP::substr($name, 7) = 'gap';        # raises an exception

  An alternative to using Char::EUCJP::substr() as an lvalue is to specify the replacement
  ...
  my $s = "The black cat climbed the green tree";
  my $z = Char::EUCJP::substr $s, 14, 7, "jumped from";    # climbed
  # $s is now "The black cat jumped from the green tree"

  Note that the lvalue returned by the three-argument version of Char::EUCJP::substr() acts
...
=item * Index by EUC-JP Character
  ...
  $index = Char::EUCJP::index($string,$substring,$offset);
  $index = Char::EUCJP::index($string,$substring);
  ...
  $pos = -1;
  while (($pos = Char::EUCJP::index($string, $lookfor, $pos)) > -1) {
      print "Found at $pos\n";
      $pos++;
  }
...
=item * Rindex by EUC-JP Character
  ...
  $rindex = Char::EUCJP::rindex($string,$substring,$offset);
  $rindex = Char::EUCJP::rindex($string,$substring);

  This subroutine works just like Char::EUCJP::index except that it returns the character
  position of the last occurrence of $substring in $string (a reverse Char::EUCJP::index).
  ...
  $pos = Char::EUCJP::length($string);
  while (($pos = Char::EUCJP::rindex($string, $lookfor, $pos)) >= 0) {
      print "Found at $pos\n";
      $pos--;
  }
...
=item * Filename Globbing
  ...
  This operator is implemented via the Char::Eeucjp::glob() subroutine. See Char::Eeucjp::glob
  of Char/Eeucjp.pm for details.
...
=item * Ordinal Value of Byte
  ...
  of "use Char::EUCJP qw(ord);" exists or not. If $expr is omitted, it uses $_.
  The return value is always unsigned.
...
=item * Reverse List or Byte String
  ...
  "use Char::EUCJP qw(reverse);" exists or not.
...
=head1 Un-Escaping bytes::* Subroutines (Char/EUCJP.pm provides)

Char/EUCJP.pm removes 'bytes::' at head of subroutine name.
...
'use utf8;' to 'use Char::EUCJP;' in its. You need help yourself for now.
...
=head1 Ignore Pragmas and Modules
  ...
  Comment out pragma to ignore utf8 environment, and Char/Eeucjp.pm provides these
  functions.
...
=item * Char::EUCJP::substr as Lvalue

If Perl version is older than 5.14, Char::EUCJP::substr differs from CORE::substr, and
...
Char::EUCJP::substr($string, 13, 4, "JPerl");
...
=item * Special Variables $` and $& need /( Capture All )/
  ...
  Before          After                Works as
  -------------------------------------------------------------------------------------------
  $`              Char::Eeucjp::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $PREMATCH       Char::Eeucjp::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  ${^PREMATCH}    Char::Eeucjp::PREMATCH()    CORE::substr($&,0,CORE::length($&)-CORE::length($1))
  $&              Char::Eeucjp::MATCH()       $1
  $MATCH          Char::Eeucjp::MATCH()       $1
  ${^MATCH}       Char::Eeucjp::MATCH()       $1
  $'              Char::Eeucjp::POSTMATCH()   $'
  $POSTMATCH      Char::Eeucjp::POSTMATCH()   $'
  ${^POSTMATCH}   Char::Eeucjp::POSTMATCH()   $'
  -------------------------------------------------------------------------------------------
...
Char::EUCJP software remains one language and one interpreter.
