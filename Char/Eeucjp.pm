...
BEGIN {
    ...
    sub Char::Eeucjp::binmode(*;$);
    sub Char::Eeucjp::open(*;$@);
    ...
    if ($] < 5.006) {
        ...
        if ($^O ne 'MacOS') {
            *CORE::GLOBAL::binmode =
            *CORE::GLOBAL::binmode = \&Char::Eeucjp::binmode;
        }
        *CORE::GLOBAL::open    =
        *CORE::GLOBAL::open    = \&Char::Eeucjp::open;
    }
}
...
if ($^O =~ /\A (?: MSWin32 | NetWare | symbian | dos ) \z/oxms) {
    if ($ENV{'ComSpec'} =~ / (?: COMMAND\.COM | CMD\.EXE ) \z /oxmsi) {
        ...
        for (@ARGV) {
            ...
            elsif (m/\A (?:$q_char)*? [*?] /oxms and (my @glob = Char::Eeucjp::glob($_))) {
                push @argv, @glob;
            }
            else {
                push @argv, $_;
...
#
{
    ...
    @Char::Eeucjp::m_matched = (qr/(?{Char::Eeucjp::m_matched})/);
    @Char::Eeucjp::s_matched = (qr/(?{Char::Eeucjp::s_matched})/);
}
...
sub _octets {
    ...
    if ($length == 1) {
        ...
        if (((caller(1))[3] ne 'Char::Eeucjp::_octets') and ($modifier =~ m/i/oxms)) {
            if ($a == $z) {
...
=item Split string
  ...
  $string on every match. If the pattern doesn't match at all, Char::Eeucjp::split returns
...
=item Chop string
  ...
  The Char::Eeucjp::chop operator is used primarily to remove the newline from the end of
  an input record but is more efficient than s/\n$//. If no argument is given, the
  function chops the $_ variable.

=item Index string

  $pos = Char::Eeucjp::index($string,$substr,$position);
  $pos = Char::Eeucjp::index($string,$substr);
  ...
  the Char::Eeucjp::index function returns -1.

=item Reverse index string

  $pos = Char::Eeucjp::rindex($string,$substr,$position);
  $pos = Char::Eeucjp::rindex($string,$substr);

  Works just like Char::Eeucjp::index except that it returns the position of the last
...
=item Make character
  ...
  set. For example, Char::Eeucjp::chr(65) is "A" in either ASCII or EUC-JP, and
  Char::Eeucjp::chr(0x82a0) is a EUC-JP HIRAGANA LETTER A. For the reverse of Char::Eeucjp::chr,
  use Char::EUCJP::ord.
...
=item binary mode (Perl5.6 emulation on perl5.005)
  ...
  Char::Eeucjp::binmode(FILEHANDLE, $disciplines);
  Char::Eeucjp::binmode(FILEHANDLE);
  Char::Eeucjp::binmode($filehandle, $disciplines);
  Char::Eeucjp::binmode($filehandle);
...
=item open file (Perl5.6 emulation on perl5.005)
  ...
  $rc = Char::Eeucjp::open(FILEHANDLE, $mode, $expr);
  $rc = Char::Eeucjp::open(FILEHANDLE, $expr);
  $rc = Char::Eeucjp::open(FILEHANDLE);
  $rc = Char::Eeucjp::open(my $filehandle, $mode, $expr);
  $rc = Char::Eeucjp::open(my $filehandle, $expr);
  $rc = Char::Eeucjp::open(my $filehandle);
  ...
  * three arguments
  ...
  {
      my $fh;                          # (uninitialized)
      Char::Eeucjp::open($fh, ">logfile")     # $fh is autovivified
  ...
  Char::Eeucjp::open my $fh, ">logfile" or die ...
  ...
  Char::Eeucjp::open(LOG, ">", "logfile") or die "Can't create logfile: $!";
  ...
  $path =~ s#^([ ])#./$1#;
  Char::Eeucjp::open (FH, "< $path\0") or die "can't open $path: $!";
  ...
  Char::Eeucjp::open(FH, "<", $path) or die "can't open $path: $!";
  ...
  Char::Eeucjp::open(FH, "<:raw", $path) or die "can't open $path: $!";
  ...
  Char::Eeucjp::open(FH, "<:crlf:encoding(Char::EUCJP)", $path) or die "can't open $path: $!";
